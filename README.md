...
🌐 **Project page:** https://anyejun.github.io/mamba-quant-compression/

---

## TL;DR

A pretrained Mamba's associative-recall failure is, in part, a **control**
problem, not only a **storage** problem. Without any fine-tuning, scaling the
pre-softplus input to the **Δ gate** at positions downstream of a target slows
the decay that target experiences and recovers recall under capacity pressure.
The control is **localized to layers 16–23**: intervening on just those 8 of 48
layers recovers most of the gain at **near-zero language-modeling cost**. A
fine-grained ablation narrows this to a **4-layer minimal core at L19–22** that
matches the 8-layer band and is both sufficient and necessary.
...
to reallocate (see [`src/pcr_writequant.py`](src/pcr_writequant.py), a deliberate
negative result).
...
│   ├── PCR_paper.pdf
│   ├── gen_figures.py            # regenerates nfig_*.pdf
│   └── nfig_{dose,layers,quant}.pdf
├── slides/                        # proposal + final presentation decks
├── docs/
...
> [`.github/workflows/pages.yml`](.github/workflows/pages.yml). To enable it once,
...
Then open [`notebooks/PCR_from_scratch.ipynb`](notebooks/PCR_from_scratch.ipynb)
...
To run the storage-vs-selectivity probe, paste
[`src/pcr_stage0_probe.py`](src/pcr_stage0_probe.py) after the notebook
(it reuses the notebook's harness).
