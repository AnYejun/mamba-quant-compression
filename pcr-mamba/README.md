...
most of the gain at **near-zero language-modeling cost**.
...
- **Single model.** Positive results are Mamba-790m only; the circuit does **not**
  replicate on Mamba-1.4b at a comparable pressure regime → likely model-specific.
- **Synthetic task.** Robust on random-token MQAR; **fragile** on natural-language
  multi-fact recall.
- **Graded priority is not established** (confounded with query position).

These bound the claim; we report them on purpose.
