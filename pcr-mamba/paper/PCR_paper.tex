...
band: L19--22 achieves recall $0.70$ ($\Delta{+}0.17$), matching the full 8-layer
result within sampling noise (the nominal $+0.03$ gap is not significant;
Experiment~F, Table~\ref{tab:minimal}). Earlier windows are weaker
...
the gain to $\Delta{-}0.10$ below base (Table~\ref{tab:main}, knockout row).

\begin{table}[t]
\centering
\caption{Minimal-circuit comparison at $N{=}32$, $g{=}0.8$. The 4-layer sub-band
L19--22 matches L16--23 (8 layers) at nearly identical PPL cost---the $+0.03$ gap
is within noise (Table~\ref{tab:ci})---and is the minimal effective circuit.}
\label{tab:minimal}
\begin{tabular}{lccc}
\toprule
...
\paragraph{Capacity sweep and circuit robustness (Experiment~G).}
The full sweep (Table~\ref{tab:main}) compares all four interventions across
$N\in\{16,32,64\}$. The 4-layer core matches the 8-layer band at $N{=}16$ and at the
$N{=}32$ operating point, but \emph{regresses below base} under high pressure
($0.35 < 0.40$ at $N{=}64$) while L16--23 reaches its strongest advantage ($+0.20$):
L19--22 is the efficient choice at the operating point, L16--23 the robust choice
above it. This protected--vs--base gap widens with load---a dedicated sweep over
$N\in\{8,\dots,64\}$ (Experiment~H) finds it persists at every level and peaks at
$N{=}64$ ($+0.20$ absolute, $+50\%$ relative)---confirming that downstream suppression
becomes a stronger retention lever as state capacity saturates.
...
is $0.620$ $[0.54, 0.69]$; the $n{=}30$ estimate of $0.53$ thus fell on the low side
of the sampling distribution. Point estimates shift up roughly uniformly at $n{=}150$,
but the intervention \emph{deltas}---on which our claims rest---are preserved. We
therefore report $n{=}30$ as the consistent regime throughout and treat this table as
its validation.
