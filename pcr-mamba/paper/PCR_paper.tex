...
($\times 1.08$ vs.\ $\times 1.07$), and recall that matches the 8-layer band
(confirmed equivalent at $n{=}150$, Experiment~F; Table~\ref{tab:ci}).
The surrounding layers L16--18 and L23 dilute rather than contribute.
Experiment~G further confirms that L19--22 is not merely sufficient but
\emph{necessary}: removing it from L16--23---leaving only layers~$\{16,17,18,23\}$---collapses
the gain to $\Delta{-}0.10$ below base (Table~\ref{tab:unified}).
...
\paragraph{Unified capacity sweep and circuit robustness (Experiment~G).}
Table~\ref{tab:unified} extends Experiment~A to compare L16--23 and L19--22 side by
side across capacity levels. At $N{=}16$ and $N{=}32$, the 4-layer circuit matches or
slightly exceeds the 8-layer band ($+0.10$ vs.\ $+0.07$ at $N{=}16$; $+0.17$ vs.\
$+0.13$ at $N{=}32$). Under high pressure ($N{=}64$), however, L19--22
\emph{regresses below base} ($0.35 < 0.40$) while L16--23 achieves its strongest
advantage ($+0.20$). The 4-layer core is optimally efficient at the $N{=}32$
operating point but not robust under sustained load; L16--23 is the reliable choice
above $N{=}32$. The necessity test (Experiment~G-2) closes the circuit argument:
removing L19--22 from L16--23 collapses recall to $\Delta{-}0.10$ below base,
establishing L19--22 as both sufficient and necessary.

\begin{table}[t]
\centering
\caption{Unified capacity sweep ($g{=}0.8$; $n{=}30$ for $N\le 32$, $n{=}20$ for
$N{=}64$). L19--22 matches or slightly exceeds L16--23 at moderate $N$ but regresses
below base at $N{=}64$; L16--23 maintains a positive advantage at every level. The
knockout row tests necessity at $N{=}32$.}
\label{tab:unified}
\begin{tabular}{lcccc}
\toprule
$N$ & Base & L16--23 (8L) & L19--22 (4L) & All layers \\
\midrule
16 & 0.77 & 0.83 ($+0.07$) & 0.87 ($+0.10$) & 0.90 ($+0.13$) \\
32 & 0.53 & 0.67 ($+0.13$) & 0.70 ($+0.17$) & 0.73 ($+0.20$) \\
64 & 0.40 & \textbf{0.60} ($\mathbf{+0.20}$) & 0.35 ($-0.05$) & 0.55 ($+0.15$) \\
\midrule
\multicolumn{5}{l}{\textit{Necessity test at $N{=}32$:}} \\
Knockout $\{16,17,18,23\}$ & 0.53 & \multicolumn{3}{c}{0.43 ($\Delta{-}0.10$) — L19--22 removed} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Statistical validation (Experiment~F).}
All preceding estimates use $n{=}30$ trials, yielding binary-proportion standard
errors of ${\approx}\pm0.09$. To validate the load-bearing comparisons at $N{=}32$,
we re-ran the three key conditions at $n{=}150$ (Table~\ref{tab:ci}). Both circuits
lift recall substantially above base (L16--23: $0.780$ $[0.71, 0.84]$; L19--22:
$0.773$ $[0.70, 0.83]$). The two intervals overlap heavily (difference $0.007$),
confirming that the $0.03$ gap in $n{=}30$ estimates reflects sampling variance
rather than a reliable difference; we characterise L19--22 as \emph{matching} rather
than exceeding L16--23. The dose--response optimum at $g{=}0.8$ is confirmed
($0.810$ $[0.72, 0.88]$ at $n{=}100$, all layers), and the base recall at $n{=}150$
is $0.620$ $[0.54, 0.69]$, indicating the $n{=}30$ estimate of $0.53$ was a
lower-variance sample; relative gains are consistent across both $n$ values.
...
\paragraph{Differential retention under capacity pressure (Experiment~H).}
Experiment~A observed that the L16--23 advantage grows with $N$ ($+0.07, +0.13, +0.20$
at $N{=}16, 32, 64$); Experiment~H formalises this as a protection--pressure curve
across $N \in \{8, 16, 24, 32, 48, 64\}$. The gap between protected recall (L16--23,
$g{=}0.8$) and unprotected base persists at every capacity level tested and peaks at
$N{=}64$ ($+0.20$ absolute, $+50\%$ relative). Although the gap is not strictly
monotone across intermediate $N$ (reflecting $n{=}20$--$30$ sample variance), the
pattern is consistent: as state capacity saturates, downstream suppression becomes a
relatively stronger retention lever, recovering half the headroom lost between $N{=}8$
and $N{=}64$.
...
combined vs.\ sum $\Delta{+}0.63$) because JRT already saturates recall at
$N\le 32$.
...
gains while always querying the earliest pair, confounding og level with write
...
the assigned og level, marginalizing over write position.
...
clean causal signal: the og level is the active lever, not position.
...
intervention); the differential og contributes minimally to that gap.
...
baseline, confirming og as the causal lever. The original confounded result is
dominated by write-position, not og.}
\label{tab:graded}
\begin{tabular}{lcccc}
\toprule
...
(Experiment~E) confirms that og level causally controls retention independent of
